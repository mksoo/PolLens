 ...
 */
export async function scrapeProvincial(config: ScraperConfig, dataDir: string): Promise<void> {
  // ⚠️ 실제 URL 패턴으로 교체 필요
  const listingUrl = `https://policy.nec.go.kr/candidate/list?electionType=도의원&district=${encodeURIComponent(config.district)}`;
